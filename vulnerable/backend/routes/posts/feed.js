...
function register(router) {
      ...
      if (type === 'people') {
        usersResult = await pool.query(
          `SELECT id, username, profile_image, bio 
           FROM users 
           WHERE username ILIKE '%${q}%' OR bio ILIKE '%${q}%' 
           ORDER BY username 
        ...
        postsResult = await pool.query(
          `SELECT p.* FROM posts p 
           WHERE p.content ILIKE '%${q}%' AND p.root_id IS NULL 
        ...
        usersResult = await pool.query(
          `SELECT id, username, profile_image, email, bio
           FROM users 
           WHERE (username ILIKE '%${q}%' OR bio ILIKE '%${q}%') AND id != ${req.user.userId}
           ORDER BY username 
           LIMIT ${PAGINATION.SEARCH_USERS_LIMIT}`
        );
      } else {
        postsResult = await pool.query(
          `SELECT p.* FROM posts p 
  ...
  router.get('/', authMiddleware, async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `SELECT * FROM posts WHERE root_id IS NULL ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`
      );
    ...
    try {
      ...
      const result = await pool.query(
        `
            ...
            FROM posts p
            LEFT JOIN reposts r ON p.id = r.post_id AND r.user_id = ${req.user.userId}
            WHERE (p.user_id = ${req.user.userId} OR r.user_id = ${req.user.userId}) 
  ...
  router.get('/user/:id', optionalAuth, async (req, res) => {
    ...
    try {
      ...
      const result = await pool.query(
        `SELECT * FROM posts WHERE user_id = ${userId} AND root_id IS NULL ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`
      );
    ...
    try {
      ...
      const result = await pool.query(
        `
            ...
            FROM posts p
            LEFT JOIN reposts r ON p.id = r.post_id AND r.user_id = ${req.user.userId}
            WHERE (p.user_id IN (SELECT following_id FROM followers WHERE follower_id = ${req.user.userId}) OR r.user_id = ${req.user.userId}) 
              AND p.root_id IS NULL
            ORDER BY COALESCE(r.created_at, p.created_at) DESC
            LIMIT ${limit} OFFSET ${offset}
        `
