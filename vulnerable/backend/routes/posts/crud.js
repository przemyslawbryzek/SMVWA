...
function register(router) {
  router.post('/', authMiddleware, async (req, res) => {
    try {
      ...
      const attachmentsLiteral = `ARRAY[${(attachment_urls || []).map(u => `'${u}'`).join(',')}]::text[]`;
      const sql = `INSERT INTO posts (user_id, content, attachments, root_id, parent_id, citation_id) VALUES (${req.user.userId}, '${content}', ${attachmentsLiteral}, ${root_id || null}, ${parent_id || null}, ${citation_id || null}) RETURNING *`;
  ...
  router.put('/:id', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      const attachmentsLiteral = `ARRAY[${attachments.map(u => `'${u}'`).join(',')}]::text[]`;
      const updateResult = await pool.query(
        `UPDATE posts SET content = '${content}', attachments = ${attachmentsLiteral} WHERE id = ${postId} RETURNING *`
      );
