...
async function toggleUserPostAction(table, userId, postId) {
  const check = await pool.query(`SELECT id FROM ${table} WHERE user_id = ${userId} AND post_id = ${postId}`);
  if (check.rows.length > 0) {
    await pool.query(`DELETE FROM ${table} WHERE user_id = ${userId} AND post_id = ${postId}`);
    return false;
  } else {
    await pool.query(`INSERT INTO ${table} (user_id, post_id) VALUES (${userId}, ${postId})`);
    return true;
  }
}
...
function register(router) {
  ...
  router.post('/:id/report', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      await pool.query(`INSERT INTO reported_posts (user_id, post_id, reason) VALUES (${req.user.userId}, ${postId}, '${reason}')`);
  ...
  router.get('/:id/comments', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      const commentsResult = await pool.query(
        `SELECT * FROM posts WHERE parent_id = ${postId} ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`
      );
  ...
  router.get('/:id/thread', authMiddleware, async (req, res) => {
    const postId = req.params.id;
    try {
      const postResult = await pool.query(`SELECT * FROM posts WHERE id = ${postId}`);
      ...
      const threadResult = await pool.query(
        `
            WITH RECURSIVE ancestors AS (
                ...
                JOIN posts child ON child.parent_id = p.id AND child.id = ${postId}
                UNION ALL
                SELECT p.* FROM posts p
