...
router.get('/profile/:id', optionalAuth, async (req, res) => {
  ...
  try {
    const sql =
      'SELECT id, username, email, profile_image, background_image, bio, created_at FROM users WHERE id = $1';
...
router.get('/suggestions', authMiddleware, async (req, res) => {
  try {
    const sql =
      'SELECT id, username, email, profile_image, background_image FROM users WHERE id != $1 ORDER BY created_at LIMIT $2';
...
router.post('/:id/report', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const sql =
      'INSERT INTO reported_users (reporting_user_id, reported_user_id, reason) VALUES ($1, $2, $3)';
